...
def main():
    ...
    mario = Agent(env=env, hparameters_x=parameters_x)

    if args.mode == 'train':
        # Get the paramters and confirm
        if args.algorithm == 'q_learning':
            mario.train_qlearning(render=args.render)
            ...
            if ans.upper() == 'N' or ans.upper() == 'NO':
                ...
                exit(0)
            mario.train_dqn(readout=readout, s=s, y=y, a=a, saver=saver, sess=sess, train_step=train_step, h_fc1=h_fc1, render=args.render)
    elif args.mode == 'play':
        checkpoint = tf.train.get_checkpoint_state("saved_networks")
        if checkpoint and checkpoint.model_checkpoint_path:
            saver.restore(sess, checkpoint.model_checkpoint_path)
            print("Successfully  loaded: ", checkpoint.model_checkpoint_path)
            mario.play_dqn(readout=readout, s=s)
